<template>
  ...
  <br/>
      ...
      >
        ...
        <v-window v-model="tab">
          ...
          stripedRows>
              ...
              <Column field="TIER" header="Liga"></Column>
    ...
    <v-card-title >
      ...
      <v-card-text>
        <div> Los 3 que han sumado más puntos a lo largo de las 6 snapshots se clasificarán a las finales nacionales.</div>
      </v-card-text>
   </v-card>
    ...
    <v-card-title >
      ...
      <v-card-text>
        <div>El ganador de cada Master Cup clasifica a las finales nacionales, y los 2 primeros clasificados, y el ganador
          de la High Level, se clasifican a la GSC.
          Del top 3 al 80 sumarán puntos para el Ranking LVLUP Teamfight Tactics.</div>
      </v-card-text>
   </v-card>
    ...
    <v-card-title >
      ...
      <v-card-text>
        <div> Los 2 que han sumado más puntos a lo largo de las Master Cups se clasificarán a las finales nacionales (aparte de los 3
          clasificados directos por top1 de Master Cup).</div>
      </v-card-text>
   </v-card>
  ...
  export default{
    setup() {
      ...
      const cups =  ref(['MasterCup 1', 'High Level 1']);
  
          watch(selectedSnap, (newValue) => fetch("https://api.laespatula.net/snapshotsESP/11/" + newValue )
              .then(res => res.json())
              .then(data => itemsSnap.value = data.datos))
  
          watch(selectedCup, (newValue) => fetch("https://api.laespatula.net/cupsESP/11/" + newValue )
              .then(res => res.json())
              .then(data => itemsCup.value = data.datos))
  
          onMounted(() => {
              fetch("https://api.laespatula.net/totalSnapESP/11").then(res => res.json())
              .then(data => itemsLadder.value = data.datos);
              fetch("https://api.laespatula.net/totalCupsESP/11").then(res => res.json())
