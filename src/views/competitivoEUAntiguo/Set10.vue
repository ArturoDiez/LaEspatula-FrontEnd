...
export default{
  setup() {
    ...
    const cups =  ref(['GSC 1', 'GSC 2']);
        ...
        watch(selectedSnap, (newValue) => fetch("https://api.laespatula.net/snapshotsEU/10/" + newValue )
            .then(res => res.json())
            .then(data => itemsSnap.value = data.datos))

        watch(selectedCup, (newValueCup) => fetch("https://api.laespatula.net/GSCEU/10/" + newValueCup )
            .then(res => res.json())
            .then(data => itemsCup.value = data.datos))

        onMounted(() => {
            fetch("https://api.laespatula.net/totalSnapEU/10").then(res => res.json())
            .then(data => itemsLadder.value = data.datos);
            fetch("https://api.laespatula.net/totalGSCEU/10").then(res => res.json())
