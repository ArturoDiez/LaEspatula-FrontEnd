...
export default{
  setup() {
    ...
    const cups =  ref(['GSC 1', 'GSC 2', 'GSC 3']);
        ...
        watch(selectedSnap, (newValue) => fetch("https://api.laespatula.net/snapshotsEU/9/" + newValue )
            .then(res => res.json())
            .then(data => itemsSnap.value = data.datos))

        watch(selectedCup, (newValueCup) => fetch("https://api.laespatula.net/GSCEU/9/" + newValueCup )
            .then(res => res.json())
            .then(data => itemsCup.value = data.datos))

        onMounted(() => {
            fetch("https://api.laespatula.net/totalSnapEU/9").then(res => res.json())
            .then(data => itemsLadder.value = data.datos);
            fetch("https://api.laespatula.net/totalGSCEU/9").then(res => res.json())
